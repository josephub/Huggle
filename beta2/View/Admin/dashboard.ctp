...
<div class="content">
	...
	foreach ($featuredPromos as $promo){ 
		if ($first){
	?>
			<div id="featured" couponId="<?php echo $promo['CouponType']['id']; ?>" class="slider" style="background: url('<?php echo $this->webroot; ?>files/uploads/<?php echo $promo['Campaign']['image']; ?>') right no-repeat;">
	...
	<?php } ?>
			<div class="footbox">
				<a href="javascript:renderSelectedPromo(<?php echo $promo['CouponType']['id']; ?>);">
					...
					<img src="<?php echo $this->webroot; ?>files/uploads/<?php echo $promo['Campaign']['image']; ?>" title ="<?php echo $promo['Campaign']['coupon_type_id']; ?>" alt="<?php echo $promo['Campaign']['coupon_type_id']; ?>" alt="" />
				</a>
			</div>
	<?php }	?>
		</div>
